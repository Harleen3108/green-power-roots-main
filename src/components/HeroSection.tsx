...
import heroImage from "@/assets/hero-biodiesel.jpg";

const slides = [
  { title: "Powering a Greener Future", accent: ["Greener", "Future"], desc: "High-performance Biodiesel Plants engineered for sustainable energy and maximum efficiency." },
  { title: "Zero Waste Engineering", accent: ["Zero", "Waste"], desc: "Turn raw feedstock into premium biofuel with 99% conversion rates and eco-friendly technology." },
  { title: "End-to-End Industrial Solutions", accent: ["End-to-End"], desc: "Strategic guidance from raw material sourcing to final market selling for plant owners." }
];
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#1B4332]">
      <motion.div initial={{ scale: 1.25 }} animate={{ scale: 1 }} transition={{ duration: 15, repeat: Infinity, repeatType: "reverse" }} className="absolute inset-0">
        ...
        <img src={heroImage} alt="Biodiesel Plant" className="w-full h-full object-cover" />
      </motion.div>
      ...
      <div className="relative z-20 container mx-auto px-6 text-center pt-32 md:pt-48 pb-16">
        <AnimatePresence mode="wait">
          <div key={index}>
            {/* 🟢 REFINED SCALE: text-3xl on mobile is much cleaner */}
            <motion.h1 
        ...
        <motion.div className="flex flex-col sm:flex-row gap-4 md:gap-6 justify-center mb-12 md:mb-20">
          <Link to="/products" className="group bg-[#74C69D] text-[#1B4332] font-black px-8 md:px-12 py-3.5 md:py-5 rounded-full text-base md:text-xl shadow-2xl transition-all flex items-center justify-center gap-2">
            Explore Plants <ArrowRight size={20} />
          </Link>
        ...
        {/* 🟢 REFINED TAGLINE: Extra small for elite industrial branding */}
