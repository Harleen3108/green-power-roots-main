...
import { Menu, X, ChevronDown } from "lucide-react"; // 🟢 Added Chevron for the pop-out effect
import logo from "../assets/logo.png";

const SiteNavbar = () => {
  const [isScrolled, setIsScrolled] = useState(false);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [isDropdownOpen, setIsDropdownOpen] = useState(false); // 🟢 State for nested menu
  ...
  // 🟢 The 4 Product links for your nested menu
  const productLinks = [
    { name: "Biodiesel Plants", path: "/products/biodiesel" },
    { name: "E-waste Recycling", path: "/products/e-waste" },
    { name: "Industrial Rentals", path: "/products/rentals" },
    { name: "Research & Lab", path: "/products/lab" },
  ];
    ...
    <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${
      ...
      <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
          ...
          <div className="relative">
            ...
            <img 
              ...
              className={`relative z-10 transition-all duration-500 object-contain ${
                shouldShowDarkStyle ? "h-10 md:h-16" : "h-12 md:h-28"
              }`}
            />
          </div>
        ...
        <div className="hidden lg:flex items-center gap-10">
          ...
          >
            <button className={`flex items-center gap-2 text-sm font-black uppercase tracking-widest transition-all hover:text-[#74C69D] ${shouldShowDarkStyle ? "text-[#1B4332]" : "text-white"}`}>
